...
class EvaluationEngine:
    ...
    async def _upsert_metrics(
        ...
        *,
        ...
        row = await self._current_metrics_row(subject_type, subject_id, now)
        if row:
    ...
    async def _current_metrics_row(
        self,
        subject_type: str,
        subject_id: str,
        now: datetime,
    ...
    async def _insert_metrics(
        ...
        *,
        ...
        await self.postgres.execute(
            ...
            now,
            _count_if(success),
            _count_if(not success),
            _count_if(human_takeover),
            duration_ms or 0,
            _rate_if(success),
            _rate_if(human_takeover),
        )
    ...
    async def should_auto_rollback(self, subject_type: str, subject_id: str) -> bool:
        ...
        if not row:
            return False
        return _should_rollback_metrics(row)


def _count_if(value: bool) -> int:
    return 1 if value else 0


def _rate_if(value: bool) -> float:
    return 1.0 if value else 0.0


def _should_rollback_metrics(row: Any) -> bool:
    thresholds = {
    ...
    if failure_rate > thresholds["max_failure_rate"]:
        return True
    return float(row["human_takeover_rate"] or 0) > thresholds["max_human_takeover_rate"]
